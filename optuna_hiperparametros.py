...
CONFIG = {
    ...
    # 👤 MEMBRO 3: Hiperparâmetros Iniciais (Modificados dinamicamente)
    "learning_rate": 2e-5,
    "batch_size": 16,
    "weight_decay": 0.01,
...
# 🔮 Variável global para transportar o estado do trial até o inicializador do modelo
CURRENT_TRIAL = None

# =====================================================================
# 🎛️ ESPAÇO DE BUSCA (EIXO MEMBRO 3)
# =====================================================================
...
def model_init():
    ...
    # Se estivermos rodando dentro do Optuna, captura os parâmetros estruturais
    if CURRENT_TRIAL is not None and CONFIG["use_custom_architecture"]:
        CONFIG["arch_pooling_strategy"] = CURRENT_TRIAL.suggest_categorical("arch_pooling_strategy", ["cls", "mean", "concat_4"])
        CONFIG["arch_freeze_layers"] = CURRENT_TRIAL.suggest_categorical("arch_freeze_layers", [0, 6, 12])
        CONFIG["arch_dropout_rate"] = CURRENT_TRIAL.suggest_float("arch_dropout_rate", 0.1, 0.5)
        CONFIG["arch_hidden_dimension"] = CURRENT_TRIAL.suggest_categorical("arch_hidden_dimension", [0, 256])
...
# =====================================================================
# 🏃‍♂️ PIPELINE PRINCIPAL CORRIGIDO
# =====================================================================
def main():
    global CURRENT_TRIAL
    ...
    # 🛠️ FUNÇÃO OBJETIVO INTERNA (PROJETADA PARA REDEFINIR O LIFECYCLE DO TRAINER)
    def objective(trial):
        global CURRENT_TRIAL
        CURRENT_TRIAL = trial  # Sincroniza o trial atual com o model_init
        
        # 1. Aplica as escolhas de hiperparâmetros deste trial específico
        hp_params = my_hp_space(trial)
        for param_name, param_value in hp_params.items():
            if param_name in CONFIG:
                CONFIG[param_name] = param_value  

        # 2. INSTANCIAÇÃO DINÂMICA DO TRAINER (ESSENCIAL PARA FORÇAR O LOG DE PARAMETROS EM CADA RUN)
        local_training_args = TrainingArguments(
            ...
            per_device_eval_batch_size=hp_params["per_device_eval_batch_size"]
        )
        ...
        # 3. Executa o treino envelopado na RUN FILHA vinculada explicitamente ao Estudo Pai
        if os.getenv("MLFLOW_TRACKING_URI"):
            ...
            with mlflow.start_run(
                ...
                # Força o log explícito dos hiperparâmetros gerados pelo Optuna nesta iteração
                mlflow.log_params(hp_params)
    ...
    # 🚀 DISPARO DO ESTUDO MANUAL DO OPTUNA
    print("\n🔥 [Optuna] Iniciando busca com árvore de gerenciamento no MLflow...")
    study = optuna.create_study(direction="maximize")

    # Abre a RUN PAI, captura seu ID e força o aninhamento no ambiente global
    if os.getenv("MLFLOW_TRACKING_URI"):
        os.environ["MLFLOW_NESTED_RUN"] = "True"
        with mlflow.start_run(run_name=f"{user_name}-optuna-study") as parent_run:
    ...
    CURRENT_TRIAL = None # Desliga o transporte do trial para o treino definitivo final

    print("\n🏆 [Optuna] BUSCA CONCLUÍDA COM SUCESSO!")
    print(f"Melhor pontuação alcançada (Macro F1): {best_trial.value:.4f}")
    print(best_trial.params)

    # =====================================================================
    # 🏋️ TREINO CONSOLIDADO FINAL & ARTEFATOS
    # =====================================================================
    print("\n🏋️ Iniciando o treinamento final definitivo...")
    
    # Atualiza o CONFIG global com os campeões absolutos do estudo
    ...
    final_training_args = TrainingArguments(
        output_dir="./results",
        num_train_epochs=6, # Épocas estendidas para convergência total
        logging_dir="./logs",
        logging_steps=20,
        eval_strategy="epoch",
        save_strategy="epoch",
    ...
    if os.getenv("MLFLOW_TRACKING_URI"):
        mlflow.start_run(run_name=f"{user_name}-final-best-model")
    ...
    if os.getenv("MLFLOW_TRACKING_URI"):
        ...
        mlflow.end_run()
        print("✅ Execução concluída! Confira a árvore estruturada com todos os parâmetros salvos no DagsHub.")
